...
CLASS ZCL_BCB_RATES_ORCHESTRATOR IMPLEMENTATION.
  ...
  METHOD if_apj_rt_exec_object~execute.
    ...
    LOOP AT mt_messages INTO DATA(ls_msg).
      ...
      IF ls_msg-message_v3 IS NOT INITIAL.
        lv_text = lv_text && | { ls_msg-message_v3 }|.
      ENDIF.
      IF ls_msg-message_v4 IS NOT INITIAL.
        lv_text = lv_text && | { ls_msg-message_v4 }|.
      ENDIF.
      CONDENSE lv_text.

      " Isola cada item para que uma falha não cancele os itens seguintes
      TRY.
    ...
    " Atrela o log ao job — erro de infra não interrompe a execução
    TRY.
